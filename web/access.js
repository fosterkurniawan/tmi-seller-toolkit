(function(){
  ...
  form.addEventListener('submit',e=>{e.preventDefault();let first=null;for(const id of keys)if(show(id)&&!first)first=field(id);localizeUI();if(first){first.focus();return;}
    if(!ToolkitAccess.grant(field('marketingConsent').checked,field('phone').value)){field('access-error').textContent='Penyimpanan sesi diblokir. Izinkan penyimpanan browser untuk masuk.';localizeUI();return;}
